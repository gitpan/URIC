...
#include <uri_escapes_file.h>

int uri_scheme_file_parse(uri_t* object)
{
  char* p = object->pool;

  p = uri_parse_frag(object, p, 0);
  p = uri_parse_scheme(object, p, 0);
  p = uri_parse_netloc(object, p, URI_SCHEME_GENERIC_PARSE_SKIP_AUTH);
  if(!object->host ||
     !strcasecmp(object->host, "localhost") ||
     strlen(object->host) == 0)
    object->host = 0;

  p = uri_parse_path(object, p, 0);

  return 0;
}

int uri_scheme_file_cannonicalize(uri_t* original, uri_t* object, char* tmp, int* tmp_sizep)
{
  int tmp_size = 0;
...
void uri_scheme_file_string(uri_t* object, char** stringp, int* string_sizep, int flags)
{
  object->info &= ~URI_INFO_RELATIVE_PATH;
  uri_scheme_generic_string(object, stringp, string_sizep, flags);
}

uri_scheme_desc_t uri_scheme_file_desc = {
  /* parse */		uri_scheme_file_parse,
  /* cannonicalize */	uri_scheme_file_cannonicalize,
