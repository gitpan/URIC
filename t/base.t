...
use URI2::URL qw(url);
use URI2::Escape qw(uri_escape uri_unescape);
...
# in the URI2::URL class
#
sub URI2::URL::_expect {
...
print "Self tests for URI2::URL version $URI2::URL::VERSION...\n";
...
URI2::URL::strict(0);
# This should work after URI2::URL::strict(0)
$url = new URI2::URL "x-myscheme:something";
# Since no implementor is registered for 'x-myscheme' then it will
# be handeled by the URI2::URL::_generic class
$url->_expect('as_string' => 'x-myscheme:something');
$url->_expect('path' => 'something');
URI2::URL::strict(1);

# since the implementor is C library and no derivation is possible
# we can drop this test
## Let's try to make our URL subclass
#{
#    package MyURL;
#    @ISA = URI2::URL::implementor();
#
#    sub _parse {
#	my($self, $init) = @_;
#	$self->URI2::URL::_generic::_parse($init, qw(netloc path));
#    }
#
#    sub foo {
#	my $self = shift;
...
#URI2::URL::implementor('x-a+b.c', 'MyURL');
#URI2::URL::implementor('x-foo', 'MyURL');
#
## Now we are ready to try our new URL scheme
#$url = new URI2::URL 'x-a+b.c://foo/bar;a?b';
#$url->_expect('as_string', 'x-a+b.c://foo/bar;a?b');
#$url->_expect('path', '/bar;a?b');
#$url->foo;
#$newurl = new URI2::URL 'xxx', $url;
#$newurl->foo;
#$url = new URI2::URL 'yyy', 'x-foo:';
#$url->foo;
#
print "ok 7\n";
...
print "URI2::URL version $URI2::URL::VERSION ok\n";
...
sub parts_test {
    ...
    $url = new URI2::URL 'file://web/orig/path';
    $url->scheme('http');
    $url->path('1info');
    $url->query('key words');
    $url->frag('this');
    ...
    die "\$url->path_components returns '$p', expected '-foo-bar-'"
      unless $p eq "-foo-bar-";
    $url->path_components("", "/etc", "\001", "..", "øse", "");
    $url->_expect('full_path' => '/%2Fetc/%01/%2E%2E/%F8se/');

    # Setting undef
    $url = new URI2::URL 'http://web/p;p?q#f';
    $url->epath(undef);
    $url->equery(undef);
    $url->eparams(undef);
    $url->frag(undef);
    ...
    $url = new URI2::URL 'gopher://gopher/';
    $url->port(33);
    $url->gtype("3");
    $url->selector("S");
    $url->search("query");
    ...
    $url = new URI2::URL 'wais://host/database/wt/wpath';
    $url->database('foo');
...
#    my(@crack, $crack);
#    @crack = URI2::URL->new("http://host/path;param?query#frag")->crack;
...
#
#    @crack = URI2::URL->new("foo/bar", "ftp://aas\@ftp.sn.no/")->crack;
#    die "Cracked result should be 9 elements" unless @crack == 9;
#    $crack = join("*", map { defined($_) ? $_ : "UNDEF" } @crack);
#    print "Cracked result: $crack\n";
#    die "Bad crack result" unless
...
#
#    @crack = URI2::URL->new('ftp://u:p@host/q?path')->crack;
#    die "Cracked result should be 9 elements" unless @crack == 9;
#    $crack = join("*", map { defined($_) ? $_ : "UNDEF" } @crack);
#    print "Cracked result: $crack\n";
#    die "Bad crack result" unless
...
#
#    @crack = URI2::URL->new("ftp://ftp.sn.no/pub")->crack;    # Test anon ftp
#    die "Cracked result should be 9 elements" unless @crack == 9;
...
#
#    @crack = URI2::URL->new('mailto:aas@sn.no')->crack;
#    die "Cracked result should be 9 elements" unless @crack == 9;
#    $crack = join("*", map { defined($_) ? $_ : "UNDEF" } @crack);
#    print "Cracked result: $crack\n";
#    die "Bad crack result" unless
...
#
#    @crack = URI2::URL->new('news:comp.lang.perl.misc')->crack;
#    die "Cracked result should be 9 elements" unless @crack == 9;
#    $crack = join("*", map { defined($_) ? $_ : "UNDEF" } @crack);
#    print "Cracked result: $crack\n";
#    die "Bad crack result" unless
...
sub netloc_test {
    ...
    my $url = new URI2::URL 'ftp://anonymous:p%61ss@håst:12345';
...
sub port_test {
    ...
    $url = URI2::URL->new('http://foo/root/dir/');
    my $port = $url->port;
...
sub escape_test {
    ...
    # supply escaped URL
    $url = new URI2::URL 'http://web/this%20has%20spaces';
    ...
    $url = new URI2::URL uri_escape('http://web/try %?#" those');
    $url->_expect('as_string',
    ...
    $url = new URI2::URL 'file://h/test?ing';
    ...
    $str = $url->as_string;
    $url = new URI2::URL $str;
    die "URL changed" if $str ne $url->as_string;

    $url = new URI2::URL 'ftp:foo';
    $url->user($reserved);
    $url->host($reserved);
...
sub newlocal_test {
    ...
    $url = newlocal URI2::URL;
    $url->_expect('as_string', URI2::URL->new("file:$dir/")->as_string);

    print "Local directory is ". $url->local_path . "\n";

    # absolute dir
    chdir('/') or die $!;
    $url = newlocal URI2::URL '/usr/';
    $url->_expect('as_string', 'file:/usr/');

    # absolute file
    $url = newlocal URI2::URL '/vmunix';
    ...
    # Relative files
    $url = new URI2::URL 'file:foo/b%61r/Note.txt';
    ...
    $url = new URI2::URL 'file://vms.host.edu/disk$user/my/notes/note12345.txt';
...
sub absolute_test {
    ...
    print "Test relative/absolute URI2::URL parsing:\n";

    # Tests from draft-ietf-uri-relative-url-06.txt
    # Copied verbatim from the draft, parsed below

    @URI2::URL::g::ISA = qw(URI2::URL::_generic); # for these tests
...
EOM
    ...
    push(@absolute_tests, ['x/y//../z' => 'http://a/b/c/x/z'],
			  ['1'         => 'http://a/b/c/1'    ],
			  ['0'         => 'http://a/b/c/0'    ],
			  ['/0'        => 'http://a/0'        ],
	...
	my($rel, $abs) = @$test;
	my $abs_url = new URI2::URL $abs;
	my $abs_str = $abs_url->as_string;

	printf("  %-10s  +  $base  =>  %s\n", $rel, $abs);
	my $u   = new URI2::URL $rel, $base;
	my $got = $u->abs;
	$got->_expect('as_string', $abs_str);
    }

    # bug found and fixed in 1.9 by "J.E. Fritz" <FRITZ@gems.vcu.edu>
    $base = new URI2::URL 'http://host/directory/file';
    my $relative = new URI2::URL 'file', $base;
    ...
    $base = new URI2::URL 'http://host/dir1/../dir2/file';
    $relative = new URI2::URL 'file', $base;
    $result = $relative->abs;
...
#
# empty gopher is an error .
#	 ["gopher:",     "",               "gopher:"],
	 ["?foo",        "http://abc/a",   "http://abc/a?foo"],
	 ["?foo",        "file:/abc",      "file:/?foo"],
	 ["#foo",        "http://abc/a",   "http://abc/a#foo"],
#	 ["#foo",        "file:a",         "file:a#foo"],
# file:a is the same as file:/a
	 ["#foo",        "file:a",         "file:/a#foo"],
	 ["#foo",        "file:/a",         "file:/a#foo"],
	 ["#foo",        "file:/a",         "file:/a#foo"],
# localhost is implicit
	 ["#foo",        "file://localhost/a", "file:/a#foo"],
...
# empty is better than ./
	...
	my $rel = URI2::URL->new($abs, $base)->rel;
...
sub eq_test
{
    my $u1 = new URI2::URL 'http://abc.com:80/~smith/home.html';
    my $u2 = new URI2::URL 'http://ABC.com/%7Esmith/home.html';
    my $u3 = new URI2::URL 'http://ABC.com:/%7esmith/home.html';
